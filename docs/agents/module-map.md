...
- `src/fleet/` — Reports this agent's liveness and spend to a bosun supervisor when one is configured, and is completely inert when it is not. Started by `createAgent`'s `start()`, not by the module: forgetting it is silent — the agent runs perfectly and reports a daily spend of zero, so budget caps bind against nothing. `FLEET_SUPERVISOR_TOKEN` is sent as a bearer header when set, which a supervisor on another box requires and one on the same box does not; a refusal is logged with its status rather than swallowed. Reads per-model cost back out of `interactions` (the map `router.ts` already writes) rather than tapping the turn path, so the router spine and `core.ts` are untouched; the watermark advances only on a report the supervisor accepted.
