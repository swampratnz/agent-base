...
// one) can orient from a committed map instead of re-exploring the tree on
// every single run. That only pays off if the map is TRUE. A stale map is
// strictly worse than no map: it sends an agent confidently to a path that
// moved, and the agent has no way to tell.
//
...
// ---------------------------------------------------------------------------
// community-agent's copy assumes exactly ONE source root, `src/`. This copy
// keeps the semantics EXACTLY and takes the roots as arguments, which is what
// a workspace layout (`packages/*/src`) needs:
//
...
//
// Entries are always written and matched as repo-relative paths (`src/foo.ts`,
// `packages/core/src/agent/`), so adding a second root does not rewrite the
// entries the first root already has.
// ---------------------------------------------------------------------------
...
const srcRoots = flagValues('--src');
if (srcRoots.length === 0) srcRoots.push('src');

const mapRel = flagValues('--map').at(-1) ?? path.posix.join('docs', 'agents', 'module-map.md');
const mapPath = path.join(repoRoot, mapRel);
...
// Directories are listed with a trailing slash so a reader can tell at a
// glance whether an entry is a subsystem or a single module.
function requiredPaths() {
  const required = [];
  for (const rootRel of srcRoots) {
    const abs = path.join(repoRoot, rootRel);
    if (!existsSync(abs)) {
      console.error(`check-context-pack: source root ${rootRel} does not exist under ${repoRoot}.`);
      process.exit(1);
    }
    const entries = readdirSync(abs, { withFileTypes: true });
    for (const e of entries) {
      if (e.isDirectory()) required.push(`${path.posix.join(rootRel, e.name)}/`);
      else if (e.isFile() && e.name.endsWith('.ts')) required.push(path.posix.join(rootRel, e.name));
    }
  }
...
if (!existsSync(mapPath)) {
  console.error(`check-context-pack: no module map at ${mapRel}. Create it (or point at it with --map).`);
  process.exit(1);
}
...
if (beginAt === -1 || endAt === -1 || endAt < beginAt) {
  console.error(
    `check-context-pack: ${mapRel} is missing its checked region. It must contain ` +
...
if (argv.includes('--write')) {
  ...
  console.log(`check-context-pack --write: normalised ${next.length} entries in ${mapRel}.`);
...
for (const rel of required) {
  if (!byPath.has(rel)) {
    problems.push(
      `${rel} has no entry in ${mapRel}. A new module must be described in the same ` +
...
for (const entry of parsed) {
  if (!pathExists(entry.path)) {
    problems.push(
      `${entry.path} is described in ${mapRel} but no longer exists (or changed between ` +
...
if (problems.length > 0) {
  console.error(`check-context-pack: ${mapRel} is out of sync with the tree:`);
  for (const p of problems) console.error(`  ${p}`);
  process.exit(1);
}

console.log(
  `check-context-pack: ${mapRel} covers all ${required.length} required paths ` +
